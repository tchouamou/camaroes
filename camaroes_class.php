...
function gener_code($cmr_code)
{
      ...
      /*==================*/
function connect($cmr_db = array())
{
	if(function_exists("connect_to_db")) $this->db_connection = connect_to_db($this->config, $cmr_db);
	return $this->db_connection;
}
      /*==================*/
      /*==================*/
function tree_group($str_list = "")
{
	return calcul_tree_group($this->config, $this->db_connection, $str_list);
}
      /*==================*/
      /*==================*/
function save_conf($type = "user", $group = "")
{
	return save_config($this->config, $this->page, $this->user, $this->db_connection, $type, $group);
}
      /*==================*/
      /*==================*/
function down_attachment($file_data = "", $file_name = "", $type = "")
{
...
function load_module_need()
{
   return cmr_load_module_need($this->config, $this->language, $this->page, $this->module);
      ...
      /*==================*/
function load_notify($subject_templates = "", $begin_text = "", $action_limit = "")
{
...
return $this;
      ...
      /*==================*/
      /*==================*/
function good_file($type = "", $action = "")
{
 		$file_list = array();
 		switch($type){
	 		case "notify":
    ...
    function update_page($win_action = "", $win_pos = "") // ----
{
     return change_page($this->page, $this->module, $this->themes, $win_action, $win_pos);
      ...
      /*==================*/
function link_default($val_table)
{
	 ...
	 switch($table){
		 case "ticket":
		    ...
		    if(function_exists("ticket_link_detail")) if($mode == "link_detail") return ticket_link_detail($this->config, $this->page, $this->language, $val_table);
			if(function_exists("ticket_link_default")) return ticket_link_default($this->config, $this->page, $this->language, $val_table);
		 break;
		 case "message":
		    if(function_exists("message_link_tab")) if($mode == "link_tab") return message_link_tab($this->config, $this->page, $this->language, $val_table);
		    if(function_exists("message_link_detail")) if($mode == "link_detail") return message_link_detail($this->config, $this->page, $this->language, $val_table);
			if(function_exists("message_link_default")) return message_link_default($this->config, $this->page, $this->language, $val_table);
		 break;
		 case "download":
		    if(function_exists("download_link_tab")) if($mode == "link_tab") return download_link_tab($this->config, $this->page, $this->language, $val_table);
		    if(function_exists("download_link_detail")) if($mode == "link_detail") return download_link_detail($this->config, $this->page, $this->language, $val_table);
			if(function_exists("download_link_default")) return download_link_default($this->config, $this->page, $this->language, $val_table);
		 break;
		 case "table":
		    if(function_exists("table_link_tab")) if($mode == "link_tab") return table_link_tab($this->config, $this->page, $this->language, $val_table);
		    if(function_exists("table_link_tab")) if($mode == "link_print") return table_link_tab($this->config, $this->page, $this->language, $val_table);
		    if(function_exists("table_link_tab")) if($mode == "link_update") return table_link_tab($this->config, $this->page, $this->language, $val_table);
		    if(function_exists("table_link_detail")) if($mode == "link_detail") return table_link_detail($this->config, $this->page, $this->language, $val_table);
			if(function_exists("table_link_default")) return table_link_default($this->config, $this->page, $this->language, $val_table);
		 break;
		 default:
		    if($mode == "link_default") return cmr_link_default($this->config, $this->page, $this->language, $val_table);
		    if($mode == "link_detail") return cmr_link_detail($this->config, $this->page, $this->language, $val_table);
		    if($mode == "link_tab") return cmr_link_tab($this->config, $this->page, $this->language, $val_table);
		    if($mode == "link_update") return cmr_link_tab($this->config, $this->page, $this->language, $val_table);
		    if($mode == "link_print") return cmr_link_tab($this->config, $this->page, $this->language, $val_table);
