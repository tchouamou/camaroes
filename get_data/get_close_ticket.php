...
//         $cmr->email["recipient"] = "" . $cmr->get_conf("cmr_admin_name") . " <" . $cmr->config["cmr_from_email"] . ">\r\n";
        ...
        $cmr->output[0] = "<b>" . $cmr->translate("Email of Ticket close sended with Success ") . "</b><br /><br />" . cmr_unescape(($post->mail_title)) . "<br /><pre>" . wordwrap(cmr_unescape(($post->text)), 80) . "</pre> <br />";
    }
    // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
if($action != "only_email"){
    $cmr->output[0] = "<b>" . $cmr->translate("Ticket close with  Success ") . "</b><br /><br />" . cmr_unescape($post->mail_title) . "<br /><pre>" . wordwrap(cmr_unescape($post->text), 80) . "</pre> <br />";

    if(($action != "new_model") && ($action != "update_model")){
        $cmr->query[0] = "UPDATE " . $cmr->get_conf("cmr_table_prefix") . "ticket SET    state_now = 'closed'   WHERE number ='" . cmr_escape($post->number) . "';";
        // *************** important **********
...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$cmr->log_to_db("'" . $cmr->get_user("auth_email") . "', '" . $cmr->get_conf("cmr_table_prefix") . "ticket', '" . $id_ticket . "' ,'close'");
