...
get_view_user.php, Ver 3.03, 2011-Aug-Thu 04:00:30  
*/
...
/*
Creating the appropriate SQL string for  user
*/
switch ($check_action){
...
// 		if($value){
// 		$object = $cmr->get_column($cmr->get_conf("cmr_table_prefix") . $table_name, "uid,name,last_name", "" . $column_id, $value);
...
// 		}
		...
		*/
		$user_email = $cmr->get_column("cmr_group", "name", "id", $id_user);
		
		if($cmr->user["authorisation"] >= $cmr->get_conf("cmr_noc_type")){
		$cmr->query[0] = "delete from  " . $cmr->get_conf("cmr_table_prefix") . "user  where  id = '" . cmr_escape($id_user) . "';";
		$cmr->query[1] = "delete from  " . $cmr->get_conf("cmr_table_prefix") . "user_user where  group_name = '" . cmr_escape($user_email) . "';";
		}
		/*
		...
		*/
		...
		$cmr->email["subject"] = "" . $cmr->config["cmr_company_name3"] . ":  " . $cmr->translate("deleted") . "  user";
		$cmr->email["message"] = $cmr->translate("user") . " [$user_email]" . $cmr->translate("was_delete user by ") . " : " . $cmr->get_user("auth_email") . "\n\n\n";
		$cmr->email["message"] .= $cmr->translate("user") . "  " . $cmr->translate("delete_success") . "  " . $cmr->translate("thanks") . "  --\n";
		$cmr->email["message"] .= $cmr->get_user("auth_sign") . "\r\n";
		
		/*
		Creating the appropriate Message to be view for confirmation
		*/
		$cmr->output[0] = "<p><b>" . $cmr->translate("user") . "  [$user_email]" . $cmr->translate("delete_success") . " </b><br /><br /> " . $cmr->translate("thanks") . "  --<br /></p>";
		/*
		...
		*/
		...
		// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		$cmr->log_to_db("'" . $cmr->get_user("auth_email") . "', '" . $cmr->get_conf("cmr_table_prefix") . "user', '" . cmr_escape($id_user) . "' ,'delete'");
		// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
