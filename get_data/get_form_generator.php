...
// 		hexdec($button_color), intval("0x".$button_color"), base_convert($button_color, 16, 10);octdec(), decbin(), dechex() 
		...
		$gen->button_dest = realpath($gen->destination);
		$gen->button_text_font = $text_font;
	    ...
	    is_array($select_models) ? $list_models = $select_models : $list_models = array($select_models);
		$model_dir = $cmr->get_path("model") . "model/" . $gen->model_group . "/";
        break;
}
...
// 			*/
			$templates_notify = cmr_look_file("notify_generaror1.xml", $cmr->get_path("notify")."templates/notify/" . $cmr->get_page("language") . "/, ". $cmr->get_path("notify")."templates/notify/" . $cmr->get_page("language") . "/auto/, ". $cmr->get_path("notify")."templates/notify/auto/");
			$cmr->notify = $cmr->load_notify($templates_notify);
// 			if(($cmr->get_conf("log_to_page_generaror"))) $cmr->output[0] = $cmr->notify["to_page"];
			if(($cmr->get_conf("log_to_log_generaror"))) $cmr->output[1] = $cmr->notify["to_log"];
			if(($cmr->get_conf("log_to_email_generaror"))) $cmr->email = $cmr->notify["to_email"];
			if(($cmr->get_conf("log_to_db_generaror")));
			// if(($cmr->get_conf("log_to_sms_generaror")));
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$cmr->page["middle1"] = $cmr->get_path("module") . "modules/admin/generators.php";
