...
if (!(function_exists("cmr_get_debug"))) {
    ...
    {
        if ($value) {
            return $GLOBALS["cmr"]->debug[$value];
        }
...
if (!(function_exists("cmr_set_debug"))) {
    ...
    {
        if ($key) {
            return ($GLOBALS["cmr"]->debug[$key] = $value);
        }
...
if (!(function_exists("get_post1"))) {
    function get_post1($arg, $method = "")
    {
        $return_val = "";
        if (empty($method)) {
            $method = isset($_GET[$arg]) ? "get" : "post";
        }
...
if (!(function_exists("cmr_get_path"))) {
    function cmr_get_path($arg="", $cmr_config = array())
    {
        ...
        if (empty($arg)) {
            return $cmr_config["cmr_path"];
        }
        if ($cmr_config["cmr_" . $arg . "_path"]) {
            return $cmr_config["cmr_" . $arg . "_path"];
        }
        return "";
    }
}
...
if (!(function_exists("cmr_get_ext"))) {
    function cmr_get_ext($arg="", $cmr_config = array())
    {
        ...
        if ($cmr_config["cmr_" . $arg . "_ext"]) {
            return $cmr_config["cmr_" . $arg . "_ext"];
        }
        return "";
    }
}
...
//     if(cmr_search("/", $cmrmodule)) $cmrmodule = substr($cmrmodule, strrpos($cmrmodule, "/") + 1);
        ...
        if (!empty($cmr_config["cmr_table_prefix"])) {
            $partner = $cmr_config["cmr_table_prefix"] . "|" . $partner;
        }
...
if (!(function_exists("cmr_good_file"))) {
    function cmr_good_file($file_list = array(), $type="")
    {
        ...
        foreach ($file_list as $one_file) {
            if ($one_file) {
                $good_path = realpath(trim($one_file));
            }
            if (file_exists($good_path)) {
                ...
                if (is_file($good_path) && ($type == "file")) {
                    return $good_path;
                }
                if (is_dir($good_path) && ($type == "dir")) {
                    return $good_path;
                }
            }
        }
...
if (!(function_exists("cmr_look_file"))) {
    ...
    {
        if (empty($some_dirs)) {
            return cmr_good_file("./" . $file_name);
        }
        $file_list = array();
        $array_dir = explode(",", $some_dirs);// $division->load_themes($cmr->themes);
        foreach ($array_dir as $one_dir) {
            $file_list[] = trim($one_dir) . "/" . $file_name;
        }
...
if (!(function_exists("cmr_error_log"))) {
    function cmr_error_log($cmr_config = array(), $cmr_session = array(), $log_text)
    {
        ...
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if (empty($cmr_config["cmr_log_path"])) {
            $cmr_config["cmr_log_path"] = "";
        }
        if (empty($cmr_session["user_email"])) {
            $cmr_session["user_email"] = "guest@localhost";
        }
        ...
        if (!empty($cmr_config["cmr_use_log"])) {
            ...
            // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            if (!empty($cmr_config["cmr_log_to_email"])) {
                error_log("\n" . $log_text, 1, $cmr_config["cmr_log_to_email"]);
            }
            if (!empty($cmr_config["cmr_log_to_system"])) {
                error_log("\n" . $log_text, 0, $cmr_config["cmr_log_to_system"]);
            }
            if (!empty($cmr_config["cmr_log_to_host"])) {
...
//return parse_ini_file($file_name);
        ...
        if (!file_exists($file_name)) {
            ...
            if (is_file($file_name)) {
                $array_file_contents = file($file_name);
            }
            $run_lines = array(";");
...
if (!(function_exists("auth_method"))) {
    function auth_method($cmr_config = array(), $adress = "unknown_adress", $name = "unknown_host")
    {
        ...
        if ((defined("cmr_no_auth"))) {
            $cmr_config["cmr_no_auth"] = cmr_no_auth;
        }
        if ((defined("cmr_apache_auth"))) {
            $cmr_config["cmr_apache_auth"] = cmr_apache_auth;
        }
        if ((defined("cmr_normal_auth"))) {
            $cmr_config["cmr_normal_auth"] = cmr_normal_auth;
        }
        if ((defined("cmr_url_auth"))) {
            $cmr_config["cmr_url_auth"] = cmr_url_auth;
        }
        if ((defined("cmr_radius_auth"))) {
            $cmr_config["cmr_radius_auth"] = cmr_radius_auth;
        }
        if ((defined("cmr_other_auth"))) {
            $cmr_config["cmr_other_auth"] = cmr_other_auth;
        }
        if ((defined("cmr_ip_auth"))) {
            $cmr_config["cmr_ip_auth"] = cmr_ip_auth;
        }
        if ((defined("cmr_cookie_auth"))) {
            $cmr_config["cmr_cookie_auth"] = cmr_cookie_auth;
        }
            ...
            if (cmr_search("cmr_no_auth|cmr_apache_auth|cmr_normal_auth|cmr_ip_auth|cmr_radius_auth|cmr_url_auth|cmr_other_auth", $method)) {
                if (!($val_method)) {
                    ...
                    foreach ($array_val as $val) {
                        if ($method == "cmr_ip_auth") {
                            list($user, $val) = explode(";", $val);
                        }
                        $val = trim($val);
                        ...
                        if (($val) && (cmr_searchi($val, $name) || cmr_searchi($val, $adress) || ($val == "*") || ($val == "1") || ($val_method == "*") || ($val_method == "1"))) {
                            ...
                            if ($method == "cmr_ip_auth") {
                                $cmr_config["cmr_auth_user"] = trim($user);
                            }
                            $cmr_config[$method] = "1";
                        }
                    }
...
if (!(function_exists("cmr_preload_config"))) {
    ...
    {
        if (empty($list_conf)) {
            $list_conf = $cmr_config["cmr_preload_conf"];
        }
        if (empty($list_conf)) {
            return  $cmr_config;
        }
        if (empty($cmr_config["cmr_path"])) {
            $cmr_config["cmr_path"] = "";
        }
        if (empty($cmr_config["cmr_conf_path"])) {
            $cmr_config["cmr_conf_path"] = "";
        }
