...
$cmr->action["module_name"] = "module.php";
...
// $division->module["title"] = " Open a news module";
...
// -----------
  ...
  $division->prints["match_value_allow_groups"] = "";
    ...
    $cmr->query["t_q"] .= " AND  (" . $cmr->get_conf("cmr_table_prefix") . "groups.type<='$aut') order by user_email;";
//   -----------
//   -----------
...
//   ==============================================================
    ...
    $cmr->query["t_q_rif"] .= " AND (" . $cmr->get_conf("cmr_table_prefix") . "user_groups.state='active') ";
    $cmr->query["t_q_rif"] .= " AND (" . $cmr->get_conf("cmr_table_prefix") . "user_groups.type='contact') "; //-- company contact --
    $cmr->query["t_q_rif"] .= " AND (" . $cmr->get_conf("cmr_table_prefix") . "user.state='active') ";
    $cmr->query["t_q_rif"] .= " AND (" . $cmr->get_conf("cmr_table_prefix") . "groups.state='active') ";
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	$cmr->query["t_group"] .= " WHERE (state='active') AND " . $cmr->action["where"];
...
if(empty($todo)) $todo = "new_module";
// ==============================================================
// =======loading module id =====================================
if(!empty($cmr->post_var["module_file"])){
//     $cmr->query["t_module"] = "SELECT * FROM " . $cmr->get_conf("cmr_table_prefix") . "module WHERE id='" . $cmr->post_var["module_file"] . "'";

//     $cmr->db["result"]["t_module"] = &$cmr->db_connection->SelectLimit($cmr->query["t_module"], $cmr->get_conf("cmr_max_view")) /*, $cmr->db_connection)*/  or db_die(__LINE__  . " - "  . __FILE__ . ": " . $cmr->db_connection->ErrorMsg());
//     $module_todo = cmrdb_fetch_object($cmr->db["result"]["t_module"]);
$result_value = return_key($cmr->db_connection,  $cmr->get_conf("cmr_table_prefix") . "module", "id", "*", $cmr->post_var["module_file"]);
//     $id_input = input_hidden("<input type=\"hidden\" value=\"".$cmr->post_var["module_file"]."\" name=\"module_file\" />");
}
// ==============================================================
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  $division->prints["match_input_hidden_get"] = input_hidden("<input type=\"hidden\" value=\"new_module.php\" name=\"cmr_get_data\" />");
...
// ========================NEW================================
	...
	// $todo_state="open";
	$cmr->query["t_model_new"] = "SELECT * FROM " . $cmr->get_conf("cmr_table_prefix") . "module where (my_master='cmr_model') and ((groups_dest in ($list_group)) or (groups_dest='') or (groups_dest=NULL))  order by id,state;";
  ...
  $division->prints["match_options_modules_dest"] .= "<option value=\"get_default_data.php\">get_default_data.php</option>";
	$dir = @opendir($cmr->get_path("index") . "modules/");
	while ($file = readdir($dir)){
	    if(($file != ".") && ($file != "..") && (is_file($cmr->get_path("index") . "modules/" . $file))){
...
//   =============Base model sistem==============
//   ============================================
//   $model_title = $r_model->module["title"];
//   ============================================
//   ============================================
//   ============================================
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    $division->prints["match_value_title"]  = $r_model->module["title"];
    (empty($r_model->mail_title)) ? $mail_title =  "module:{{module_title}}" : $mail_title = $r_model->mail_title ;
    $division->prints["match_value_comment"] .= "\n* " . date("Y-m-d H:i:s") . $cmr->translate(" Write by [") . $cmr->get_user("auth_email") . "]\n";
//   empty($r_model->sender) ? $sender = $cmr->config["cmr_from_email"] : $sender = $r_model->sender;
//   $division->prints["match_value_mail_to"] = $r_model->mail_to;
...
if(empty($cmr->post_var["module_file"])){
    ...
    $cmr->query["t_model_update"] = "SELECT * FROM " . $cmr->get_conf("cmr_table_prefix") . "module where (1) ";
...
//   =======================================
    $division->prints["match_value_model_id"] = $r_model->id;
//   =======================================
    $division->prints["match_module_title1"] = $cmr->translate("update module");
//   $division->prints["match_module_title1"]  = $cmr->translate("reply_module_title");
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  $division->prints["match_value_allow_groups"] = $result_value["allow_groups"];
    ...
    $division->prints["match_submit_text"]  = $cmr->translate("reply_module");
    $division->prints["match_submit_text"]  = $cmr->translate("reply_module");
//   $division->prints["match_load_script"]  = "<script language=\"javascript\" type=\"text/javascript\">load_model(this.form,'model', 'module_');</script>";
    $division->prints["match_load_script"] ="";
    break;
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	$file_list = array();
	$file_list[] = $cmr->get_user("auth_user_path") . "templates/modules/template_module" . $cmr->get_ext("template");
	$file_list[] = $cmr->get_user("auth_group_path") . "templates/modules/template_module" . $cmr->get_ext("template");
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  $division->prints["match_label_module_alert1"] = $cmr->translate("module_alert1");
  $division->prints["match_label_module_alert2"] = $cmr->translate("module_alert2");
  $division->prints["match_label_module_alert3"] = $cmr->translate("module_alert3");
  $division->prints["match_label_module_alert4"] = $cmr->translate("module_alert4");
  $division->prints["match_label_module_alert5"] = $cmr->translate("module_alert5");
  $division->prints["match_label_module_alert6"] = $cmr->translate("module_alert6");
  $division->prints["match_label_module_alert7"] = $cmr->translate("module_alert7");
  $division->prints["match_label_module_alert8"] = $cmr->translate("module_alert8");
  $division->prints["match_label_module_alert9"] = $cmr->translate("module_alert9");
  $division->prints["match_label_module_alert10"] = $cmr->translate("module_alert10");
  $division->prints["match_label_module_alert11"] = $cmr->translate("module_alert11");
  $division->prints["match_label_module_alert12"] = $cmr->translate("module_alert12");
  $division->prints["match_label_module_alert13"] = $cmr->translate("module_alert13");
  ...
  $division->prints["match_label_module_alert19"] = $cmr->translate("module_alert19");
  $division->prints["match_label_module_alert20"] = $cmr->translate("module_alert20");
  $division->prints["match_label_module_alert21"] = $cmr->translate("module_alert21");
  $division->prints["match_label_module_alert22"] = $cmr->translate("module_alert22");
  $division->prints["match_label_module_alert23"] = $cmr->translate("module_alert23");
  $division->prints["match_label_module_alert24"] = $cmr->translate("module_alert24");
  $division->prints["match_label_module_alert25"] = $cmr->translate("module_alert25");
