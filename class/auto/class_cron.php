...
class_cron.php, Ver 3.03, 2011-Apr-Thu 23:21:42
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["cron_read"] = $cmr->readed_line("cron");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("cron_class"))){
    class cron_class{
	...
	{
	   return $this->cron_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function cron_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['command'] = $this->command : $this->cmr_array_column['command'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['at'] = $this->at : $this->cmr_array_column['at'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->command = '';
            if(get_post('command')) $this->command = get_post('command', $action, $pre_match);
            
            $this->at = '';
            if(get_post('at')) $this->at = get_post('at', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "cron WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "cron WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "cron field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "cron into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "cron WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "cron WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
