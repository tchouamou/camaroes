...
class_command.php, Ver 3.03, 2011-Apr-Thu 23:21:41
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["command_read"] = $cmr->readed_line("command");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("command_class"))){
	...
	{
	   return $this->command_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function command_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['command_name'] = $this->command_name : $this->cmr_array_column['command_name'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['command_line'] = $this->command_line : $this->cmr_array_column['command_line'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->command_name = '';
            if(get_post('command_name')) $this->command_name = get_post('command_name', $action, $pre_match);
            
            $this->command_line = '';
            if(get_post('command_line')) $this->command_line = get_post('command_line', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "command WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "command WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "command field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "command into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "command WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "command WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
