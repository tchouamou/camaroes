...
class_comment.php, Ver 3.03, 2011-Apr-Thu 23:21:41
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["comment_read"] = $cmr->readed_line("comment");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("comment_class"))){
    class comment_class{
	...
	{
	   return $this->comment_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function comment_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['encoding'] = $this->encoding : $this->cmr_array_column['encoding'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->encoding = '';
            if(get_post('encoding')) $this->encoding = get_post('encoding', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "comment WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "comment WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "comment field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "comment into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "comment WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "comment WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
