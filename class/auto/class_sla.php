...
class_sla.php, Ver 3.03, 2011-Apr-Thu 23:21:47
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["sla_read"] = $cmr->readed_line("sla");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("sla_class"))){
    class sla_class{
	...
	{
	   return $this->sla_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function sla_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['num_user_impact'] = $this->num_user_impact : $this->cmr_array_column['num_user_impact'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['num_group_impact'] = $this->num_group_impact : $this->cmr_array_column['num_group_impact'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['num_asset_impact'] = $this->num_asset_impact : $this->cmr_array_column['num_asset_impact'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->num_user_impact = '';
            if(get_post('num_user_impact')) $this->num_user_impact = get_post('num_user_impact', $action, $pre_match);
            
            $this->num_group_impact = '';
            if(get_post('num_group_impact')) $this->num_group_impact = get_post('num_group_impact', $action, $pre_match);
            
            $this->num_asset_impact = '';
            if(get_post('num_asset_impact')) $this->num_asset_impact = get_post('num_asset_impact', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "sla WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "sla WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "sla field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "sla into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "sla WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "sla WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
