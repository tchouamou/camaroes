...
class_config.php, Ver 3.03, 2011-Apr-Thu 23:21:41
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["config_read"] = $cmr->readed_line("config");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("config_class"))){
	...
	{
	   return $this->config_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function config_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['the_key'] = $this->the_key : $this->cmr_array_column['the_key'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['value'] = $this->value : $this->cmr_array_column['value'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->the_key = '';
            if(get_post('the_key')) $this->the_key = get_post('the_key', $action, $pre_match);
            
            $this->value = '';
            if(get_post('value')) $this->value = get_post('value', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "config WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "config WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "config field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "config into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "config WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "config WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
