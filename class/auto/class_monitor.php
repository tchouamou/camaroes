...
class_monitor.php, Ver 3.03, 2011-Apr-Thu 23:21:45
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["monitor_read"] = $cmr->readed_line("monitor");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("monitor_class"))){
    class monitor_class{
	...
	{
	   return $this->monitor_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function monitor_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['monitor_group'] = $this->monitor_group : $this->cmr_array_column['monitor_group'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['config'] = $this->config : $this->cmr_array_column['config'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->monitor_group = '';
            if(get_post('monitor_group')) $this->monitor_group = get_post('monitor_group', $action, $pre_match);
            
            $this->config = '';
            if(get_post('config')) $this->config = get_post('config', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "monitor WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "monitor WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "monitor field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "monitor into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "monitor WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "monitor WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
