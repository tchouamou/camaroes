...
class_session.php, Ver 3.03, 2011-Apr-Thu 23:21:47
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["session_read"] = $cmr->readed_line("session");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("session_class"))){
	...
	{
	   return $this->session_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function session_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['sessionid'] = $this->sessionid : $this->cmr_array_column['sessionid'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['sessionip'] = $this->sessionip : $this->cmr_array_column['sessionip'] = $value;
            ...
            (empty($value)) ? $this->cmr_array_column['time_out'] = $this->time_out : $this->cmr_array_column['time_out'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['session_end'] = $this->session_end : $this->cmr_array_column['session_end'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->sessionid = '';
            if(get_post('sessionid')) $this->sessionid = get_post('sessionid', $action, $pre_match);
            
            $this->sessionip = '';
            if(get_post('sessionip')) $this->sessionip = get_post('sessionip', $action, $pre_match);
            ...
            $this->time_out = '';
            if(get_post('time_out')) $this->time_out = get_post('time_out', $action, $pre_match);
            
            $this->session_end = '';
            if(get_post('session_end')) $this->session_end = get_post('session_end', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "session WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "session WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "session field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "session into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "session WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
            if(isset($this->time_out) && empty($this->time_out)) $this->time_out = date("Y-m-d H:i:s");
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "session WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
