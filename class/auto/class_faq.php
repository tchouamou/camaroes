...
class_faq.php, Ver 3.03, 2011-Apr-Thu 23:21:43
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["faq_read"] = $cmr->readed_line("faq");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("faq_class"))){
    class faq_class{
	...
	{
	   return $this->faq_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function faq_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['argoment'] = $this->argoment : $this->cmr_array_column['argoment'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['question'] = $this->question : $this->cmr_array_column['question'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['response'] = $this->response : $this->cmr_array_column['response'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->argoment = '';
            if(get_post('argoment')) $this->argoment = get_post('argoment', $action, $pre_match);
            
            $this->question = '';
            if(get_post('question')) $this->question = get_post('question', $action, $pre_match);
            
            $this->response = '';
            if(get_post('response')) $this->response = get_post('response', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "faq WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "faq WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "faq field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "faq into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "faq WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "faq WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
