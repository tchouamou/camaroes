...
class_download.php, Ver 3.03, 2011-Apr-Thu 23:21:42
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["download_read"] = $cmr->readed_line("download");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("download_class"))){
	...
	{
	   return $this->download_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function download_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['file_name'] = $this->file_name : $this->cmr_array_column['file_name'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['file_type'] = $this->file_type : $this->cmr_array_column['file_type'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['file_size'] = $this->file_size : $this->cmr_array_column['file_size'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->file_name = '';
            if(get_post('file_name')) $this->file_name = get_post('file_name', $action, $pre_match);
            
            $this->file_type = '';
            if(get_post('file_type')) $this->file_type = get_post('file_type', $action, $pre_match);
            
            $this->file_size = '';
            if(get_post('file_size')) $this->file_size = get_post('file_size', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "download WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "download WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "download field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "download into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "download WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "download WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
