...
class_rss.php, Ver 3.03, 2011-Apr-Thu 23:21:46
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["rss_read"] = $cmr->readed_line("rss");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("rss_class"))){
	...
	{
	   return $this->rss_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function rss_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['link'] = $this->link : $this->cmr_array_column['link'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['language'] = $this->language : $this->cmr_array_column['language'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['rating'] = $this->rating : $this->cmr_array_column['rating'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['state'] = $this->state : $this->cmr_array_column['state'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['feed_adress'] = $this->feed_adress : $this->cmr_array_column['feed_adress'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->link = '';
            if(get_post('link')) $this->link = get_post('link', $action, $pre_match);
            
            $this->language = '';
            if(get_post('language')) $this->language = get_post('language', $action, $pre_match);
            
            $this->rating = '';
            if(get_post('rating')) $this->rating = get_post('rating', $action, $pre_match);
            
            $this->state = '';
            if(get_post('state')) $this->state = get_post('state', $action, $pre_match);
            
            $this->feed_adress = '';
            if(get_post('feed_adress')) $this->feed_adress = get_post('feed_adress', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "rss WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "rss WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "rss field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "rss into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "rss WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "rss WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
