...
class_statistic.php, Ver 3.03, 2011-Apr-Thu 23:21:47
*/
...
// ==============================================        
    // ==========
    // ==========
    $GLOBALS["statistic_read"] = $cmr->readed_line("statistic");
    // ==========
    // ==========
// ==============================================        
if(!(class_exists("statistic_class"))){
	...
	{
	   return $this->statistic_class($cmr_config, $cmr_user);
	}
    //00000000000000000000000000 
        function statistic_class($cmr_config = array(), $cmr_user = array())
        {
        ...
        function get_array_datas($value = "")
        {
            ...
            (empty($value)) ? $this->cmr_array_column['period_begin'] = $this->period_begin : $this->cmr_array_column['period_begin'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['period_end'] = $this->period_end : $this->cmr_array_column['period_end'] = $value;
            
            (empty($value)) ? $this->cmr_array_column['data'] = $this->data : $this->cmr_array_column['data'] = $value;
        ...
        function get_form_datas($action = "post", $pre_match = "")
        {
            ...
            $this->period_begin = '';
            if(get_post('period_begin')) $this->period_begin = get_post('period_begin', $action, $pre_match);
            
            $this->period_end = '';
            if(get_post('period_end')) $this->period_end = get_post('period_end', $action, $pre_match);
            
            $this->data = '';
            if(get_post('data')) $this->data = get_post('data', $action, $pre_match);
...
//          $this->cmr_query["select"] = "SELECT * FROM " . $this->cmr_prefix . "statistic WHERE " . $this->accept("select") . " and " . $where;//." LIMIT ".$this->cmr_limit;
...
//          $this->cmr_query["view"] = "SELECT * FROM " . $this->cmr_prefix . "statistic WHERE " . $this->accept("select") . "";
...
//          $this->cmr_query["import"] = "LAOD DATA LOCAL INFILE " . $file_name . " into table " . $this->cmr_prefix . "statistic field terminate by '\\\n';";
...
//          $this->cmr_query["export"] = "SELECT * FROM " . $this->cmr_prefix . "statistic into outfile " . $file_name . " field terminate by '\\\n'";
...
//          $this->cmr_query["report"] = "SELECT COUNT * FROM " . $this->cmr_prefix . "statistic WHERE " . $this->accept("select") . " and " . $where; //." LIMIT " . $this->cmr_limit;
        ...
        {
// 		$this->cmr_query["delete"]  = "DELETE FROM " . $this->cmr_prefix . "statistic WHERE " . $this->accept("delete");
// 		
// 		$list_id = "";
// 		if(is_array($val_id)){
